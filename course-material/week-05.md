...
- political data such as death rates by gun or the positive/negative percentages of news headlines
- community opinion on an idea such as "An innocent person in prison or 10 guilty people on the streets?"
- the data from a performance, maybe you want to track your movements in a camera and use this as data?
- personal stories as data, such as Gabori's works about her experiences, perhaps you want to collect your own observations about your life such as the fish disrupting the water: how many fish were there, how long did they disrupt the water?

Or maybe you want to find an existing data set and tell a different story like I did with the Coronavirus dataset?

Or maybe you want to create your own dataset and tell your own story of lockdown? How many hours did you spend in front of a screen? How many people did you talk with in a day? Nicolas Felton does a visual [annual report](http://feltron.com/FAR11.html) of his data each year.
...
## Using data
So the first thing we're going to look at is how to use a simple dataset. 
...
p5.js has different but similar ways of handling these particular files and if you're interested in visualisations - I recommend that you start to explore these different file types as you will need them for particular datasets.
...
You would get an error in your console(Option+CMD+J) something along the lines of "Fetch API cannot load file. URL scheme must be "http" or "https" for CORS request."
...
### CSV data
So we're going to start with this super simple CSV file. [Download this CSV file](https://drive.google.com/file/d/1RaiHSx_Awp0QEkc1V9uIsLN7EV0a48uv/view?usp=sharing).
...
So first, let's get all the data from the column called 'pub'. This will give us a list of the all the pub names.
```javascript
let table;
...
So the next thing we also need to observe here is that CSV files contains data and the data is assumed to be "String" values. Remember we spoke about data types - String, Number, Boolean etc? Well, CSV files are just a bunch of values separated by commas hence why CSV stands for "Comma-separated values" - it's up to us to tell the computer what type of data it is.
...
And now let's get each pub's score and draw them out... can you guess how we can repeat through each row and get out the score?
```javascript
let table;
...
function draw() {
  ...
  for (let i = 0; i < table.getRowCount(); i++){
    ...
    ellipse(50+i*50, windowHeight/2, score, score);
  }
} 
```

So GREAT! Now we have the scores coming through and we know how to get data. It's looking a bit bleh and there's not enough difference between the scores to see it in any meaningful way. Let's look at the data more closely, by looking at the "header" row, to see what else we can use:
```
...
So perhaps instead of just a static x,y position of (50+i*50,windowHeight/2), we want the ellipse to be where it is located on a real map. So let's grab out the longitude and latitude.
```javascript
let table;
...
Option B: Make your own dataset that tells your story - maybe you start to collect some of your personal stats or you look at the Rotten Tomatoes scores of your favourite comedies. To collect your data, start with an empty spreadsheet in something like Google Sheets/Excel and "Export as a CSV".

Option C: Already started Task 2? Start to think about how you could you incorporate a dataset into your project? Have a look at some static values you are using - could you turn that into something more meaningful? 

Your task this week is to find a dataset and to connect visuals to that data. Just like we did today.
